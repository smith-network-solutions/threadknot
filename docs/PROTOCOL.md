...
| `thread.smartSearch` | `{ query, threadIds[], model? }` | `{ results: [{ threadId, snippet, reason, score }], model, rankedByModel, candidates }` — AI-ranked content search. `query` is a description of the conversation, not a keyword. The server scans the requested transcripts (user and assistant messages only) for the query terms, keeps the best lexical hits plus the most recent threads, and hands compact digests of those candidates to an ephemeral `claude -p` run (the same subscription-login path titles use) which returns matching ids with a one-line `reason` and a 0-100 `score`. `snippet` is a server-cut excerpt around a keyword hit, never model text. `model` must be one of `claude-haiku-4-5`, `claude-sonnet-5`, `claude-opus-5` (default Opus, run through `claude -p`) or `gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-6-astra` (run through `codex exec --ephemeral`); anything else is refused. If the CLI is missing or fails, the lexical ranking comes back with `rankedByModel: false`. Routes by `machineId` like `thread.search`, so each owner ranks its own transcripts and the client merges by score |
