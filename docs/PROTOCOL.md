...
| `thread.search` | `{ query, threadIds[] }` | `{ threadIds[] }` - indexed full-content search over requested local threads, with a transcript-scan fallback during backfill or index failure. Routes by optional `machineId`. Indexed matching is token-based. |
| `thread.indexedSearch` | `{ query, threadIds[], machineId? }` | `{ results: [{ threadId, snippet, reason, score, messageSeq }], index: { ready, indexedThreads, totalThreads, error } }` - up to 60 distinct threads ranked by local Tantivy search, with plain-text excerpts and event locations. No model call. Scope is intersected with the local catalog. During backfill, partial results and progress are returned. `score` is BM25, not AI confidence. See [Conversation search](CONVERSATION-SEARCH.md). |
| `thread.smartSearch` | `{ query, threadIds[], model? }` | `{ results: [{ threadId, snippet, reason, score }], model, rankedByModel, candidates }` — AI-ranked content search. `query` is a description of the conversation, not a keyword. The server retrieves up to 40 matching threads from the local Tantivy index (user/assistant messages and titles), adds up to 20 recent threads, reads only those candidate transcripts, and hands compact digests of those candidates to an ephemeral `claude -p` run (the same subscription-login path titles use) which returns matching ids with a one-line `reason` and a 0-100 `score`. `snippet` is a server-cut excerpt around a keyword hit, never model text. `model` must be one of `claude-haiku-4-5`, `claude-sonnet-5`, `claude-opus-5` (default Opus, run through `claude -p`) or `gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-6-astra` (run through `codex exec --ephemeral`); anything else is refused. During backfill or an index outage, candidate selection falls back to transcript scanning. If the CLI is missing or fails, the lexical ranking comes back with `rankedByModel: false`. Routes by `machineId` like `thread.search`, so each owner ranks its own transcripts and the client merges by score |
