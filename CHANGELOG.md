...
- **Search inside your conversations, not just their titles.** Click Search and the search takes the sidebar's place, wider than the chat list and draggable to any width. Typing filters titles instantly. Describe the chat you remember ("the one where we voided a duplicate invoice") and press Enter: a small model reads through the conversations in scope and lists the ones that match, each with a line on why and an excerpt. The model chip picks who reads: Opus 5 by default, Haiku 4.5 or Sonnet 5 if you want it cheaper, or a Codex model (Luna, Sol, Astra). It runs on your own CLI logins like thread titles do. Search starts across all projects; the scope chip narrows it to the open one.
