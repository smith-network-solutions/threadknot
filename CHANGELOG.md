...
- **The search moves into the sidebar once you pick a result.** Click any result and it opens on the right while the whole search (your words, the chips, the ranked list) takes the sidebar's place on the left. Click through the candidates or use the arrows; the open one is highlighted and the query stays editable. Drag the sidebar edge to make the search as wide or narrow as you like; it opens wider than the chat list, remembers its own width, and the chat list comes back at its usual width when you close with the X (or Escape), leaving that conversation open.
