...
- **The search moves into the sidebar once you pick a result.** Click any result and it opens on the right while the whole search (your words, the chips, the ranked list) takes the sidebar's place on the left. Click through the candidates or use the arrows; the open one is highlighted and the query stays editable. The X (or Escape) brings the chat list back and leaves that conversation open.
